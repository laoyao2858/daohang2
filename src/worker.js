    ...
    switch (resource) {
      ...
      case 'categories':
        ...
        if (request.method === 'POST' && pathParts[2] === 'order') {
          ...
          if (!Array.isArray(orderedIds)) return jsonResponse({ error: 'Invalid data format, expected orderedIds array' }, 400);
        ...
        if (request.method === 'POST' && pathParts[2] === 'update-all') {
          const { updates } = await request.json();
          if (!Array.isArray(updates)) return jsonResponse({ error: 'Invalid data format, expected updates array' }, 400);
