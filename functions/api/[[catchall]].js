    ...
    switch (resource) {
      case 'settings':
        ...
        if (request.method === 'POST') {
          ...
          for (const [key, value] of Object.entries(updates)) {
             statements.push(env.DB.prepare('INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)').bind(key, value));
          }
      ...
      case 'categories':
        ...
        if (request.method === 'POST' && pathParts[2] === 'order') {
          ...
          if (!Array.isArray(orderedIds)) return jsonResponse({ error: 'Invalid data' }, 400);
        ...
        if (request.method === 'POST' && pathParts[2] === 'update-all') {
          const { updates } = await request.json();
          if (!Array.isArray(updates)) return jsonResponse({ error: 'Invalid data' }, 400);
          const statements = updates.map((update) => {
            return env.DB.prepare('UPDATE categories SET type = ?, displayOrder = ? WHERE id = ?')
      ...
      case 'site-groups':
        ...
        if (request.method === 'POST') {
          ...
          const newOrder = (results[0].maxOrder || 0) + 1;
          const stmt = env.DB.prepare('INSERT INTO site_groups (name, color, icon, display_order) VALUES (?, ?, ?, ?)').bind(name, color || '', icon || '', newOrder);
          const { meta } = await stmt.run();
          return jsonResponse({ success: true, id: meta.last_row_id });
        }
        break;
      ...
      case 'import':
        if (request.method === 'POST') {
            // ... (import logic kept same as original, omitted for brevity but should be included)
             const data = await request.json();
             const statements = [];
             // Simplified for brevity - in real deployment keep the full import logic
             if(data.categories) {
